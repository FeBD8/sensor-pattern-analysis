...
def heatmap_offset(data, row_labels, col_labels, sim_adj, columns, dict_col_index,col_dic, ax=None,
...
def annotate_heatmap_Reds(im, data=None,data2=None, valfmt="{x:.2f}",
                     textcolors=["black", "white"],
                     threshold=0.20, **textkw):
    """
    ...
    **kwargs
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            kw.update(color=textcolors[int(im.norm(data2[i, j]) > threshold)])
            val = abs(data[i, j])
            text = im.axes.text(j, i, valfmt(val, None),fontsize=16, **kw)
            texts.append(text)

    return texts

def annotate_heatmap_GrRed(im, data=None, valfmt="{x:.2f}",
                     textcolors=["black", "white"],
                     threshold=0.3, **textkw):
    """
    A function to annotate a heatmap.
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            if data[i, j] < 0 or data[i, j] == 0 :
                kw.update(color=textcolors[int(im.norm(abs(data[i, j])) > 0)])
            else :
